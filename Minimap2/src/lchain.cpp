...
static int64_t mg_chain_bk_end(int32_t max_drop, const mm128_t *z, const int32_t *f, const int64_t *p, int32_t *t, int64_t k)
{
	...
	do {
		int32_t s;
		// fprintf(stderr, "i: %d -> %d , t: %d\n", i, p[i], t[i]);
		t[i] = 2; // ?
		end_i = i = p[i]; // backtrack the chain -> toward the forward-most anchor
		s = i < 0? z[k].x : (int32_t)z[k].x - f[i]; // Base anchor score - current anchor score 
		// fprintf(stderr, "sc: %d, s: %d \n",f[i], s);
		if (s > max_s) max_s = s, max_i = i; // find the anchor with maximum score drop
		else if (max_s - s > max_drop) break; // if s drops more than max_drop from max_s, break
	} while (i >= 0 && t[i] == 0);
...
uint64_t *mg_chain_backtrack(void *km, int64_t n, const int32_t *f, const int64_t *p, int32_t *v, int32_t *t, int32_t min_cnt, int32_t min_sc, int32_t max_drop, int32_t *n_u_, int32_t *n_v_)
{
	...
	// fprintf(stderr, "min_cnt: %d, min_sc: %d, max_drop: %d\n", min_cnt, min_sc, max_drop);
	...
	*n_u_ = *n_v_ = 0;
	...
	memset(t, 0, n * 4); // initialize t..
	for (k = n_z - 1, n_v = n_u = 0; k >= 0; --k) { // precompute n_u
		// backtrack chain starting at z[k].y
		if (t[z[k].y] == 0) { // not visited?
			int64_t n_v0 = n_v, end_i;
			int32_t sc;
			end_i = mg_chain_bk_end(max_drop, z, f, p, t, k); // compute the front-most index of the chain
			for (i = z[k].y; i != end_i; i = p[i]) 
				++n_v, t[i] = 1; // update total number of anchors in the read, mark visited anchor
				// n_v: number of chained anchors?
			sc = i < 0? z[k].x : (int32_t)z[k].x - f[i]; // ????
			if (sc >= min_sc && n_v > n_v0 && n_v - n_v0 >= min_cnt)
				++n_u; // n_u: number of valid chains?
			else n_v = n_v0;
		}
	}

	// fprintf(stderr, "n_v: %lu, n_u:%d\n", n_v, n_u);
	
	u = Kmalloc(km, uint64_t, n_u);
	memset(t, 0, n * 4);
	for (k = n_z - 1, n_v = n_u = 0; k >= 0; --k) { // populate u[]
		// fprintf(stderr, "[%d] sc: %lu, i:%lu\n", k, z[k].x, z[k].y);
		if (t[z[k].y] == 0) { 
			int64_t n_v0 = n_v, end_i;
			int32_t sc;
			end_i = mg_chain_bk_end(max_drop, z, f, p, t, k);
			for (i = z[k].y; i != end_i; i = p[i])
				v[n_v++] = i, t[i] = 1; // v: chained anchors 
			sc = i < 0? z[k].x : (int32_t)z[k].x - f[i];
			if (sc >= min_sc && n_v > n_v0 && n_v - n_v0 >= min_cnt)
				u[n_u++] = (uint64_t)sc << 32 | (n_v - n_v0); // u: score | # of anchors in a chain
			else n_v = n_v0;
		}
	}
					   ...
					   int64_t n, mm128_t *a, int *n_u_, uint64_t **_u, void *km)
{
	...
	// fprintf(stderr, "start rmq chain: %d anchors\n", n);
	//fprintf(stderr, "params max_dist: %d, max_dist_inner:%d, bw:%d, max_chn_skip:%d, cap_rmq_size: %d, min_cnt:%d, min_sc:%d, chn_pen_gap: %f, chn_pen_skip:%f\n",
	//max_dist, max_dist_inner, bw, max_chn_skip, cap_rmq_size, min_cnt, min_sc, chn_pen_gap, chn_pen_skip);
	...
	for (i = i0 = 0; i < n; ++i) {
		...
		if ((q = krmq_rmq(lc_elem, root, &lo, &hi)) != 0) {
			...
			if (width <= bw && sc > max_f) { max_f = sc, max_j = j; // fprintf(stderr, "update score & parent at %d: %d, %d, %d\n", i, sc, j, exact);
			}
			if (!exact && root_inner && (int32_t)a[i].y > 0) {
				// fprintf(stderr, "rmq inner loop: %d\n", i);
				lc_elem_t *lo, *hi;
				...
				if (lo) {
					...
					while ((q = krmq_at(&itr)) != 0) {
						...
						if (width <= bw) {
							if (sc > max_f) {
								max_f = sc, max_j = j; //fprintf(stderr, "inner update score & parent at %d: %d, %d, %d\n", i, sc, j, exact);
								if (n_skip > 0) --n_skip;
							} else if (t[j] == (int32_t)i) {
